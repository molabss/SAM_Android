 ...
 */
public class AjusteIntegranteAdapter extends AbstractAdapter<EventoEquipeVO> {

    public AjusteIntegranteAdapter(Context context, List<EventoEquipeVO> list) {
        super(context, R.layout.integrante_grid_item, list);
    }

    @Override
    protected void preencherItensGrid(View view, int i) {
        TextView eventoText = (TextView) view.findViewById(R.id.eventoText);
        TextView horaIniText = (TextView) view.findViewById(R.id.horaIniText);
        TextView horaFimText = (TextView) view.findViewById(R.id.horaFimText);
        ...
        EventoEquipeVO eventoEquipe = list.get(i);

        horaIniText.setText(eventoEquipe.getHoraIni());
        horaFimText.setText(eventoEquipe.getHoraFim());
        eventoText.setText(eventoEquipe.getParalisacao().getDescricao());

        if (eventoEquipe.getHoraFim() == null || eventoEquipe.getHoraFim().isEmpty()) {
            horaFimText.setText(" - ");
        }
